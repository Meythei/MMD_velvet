import type { MmdManager, WgslMaterialShaderPresetId } from "../core/mmd-manager";
...
export class UIController {
    ...
    private appLogoTrigger: HTMLElement;
    private languageSelect: HTMLSelectElement;
    private themeSelect: HTMLSelectElement;

    // Settings Modal
    private settingsModal: HTMLElement;
    private settingsCloseBtn: HTMLElement;
    private settingsBgColorInput: HTMLInputElement;
    ...
    private interpolationTrackNameLabel: HTMLElement | null = null;
    private interpolationFrameLabel: HTMLElement | null = null;
    private interpolationTypeSelect: HTMLSelectElement;
    private interpolationStatusLabel: HTMLElement | null = null;
    ...
    private windowTitleEl: HTMLElement | null = null;
    ...
    constructor(mmdManager: MmdManager, timeline: Timeline, bottomPanel: BottomPanel) {
        ...
        // Get DOM elements
        this.btnExportPng = document.getElementById("btn-export-png") as HTMLElement;
        ...
        this.btnToggleGround = document.getElementById("btn-toggle-ground") as HTMLElement;
        this.groundToggleText = document.getElementById("ground-toggle-text") as HTMLElement;
        this.btnToggleSkydome = document.getElementById("btn-toggle-skydome") as HTMLElement;
        this.skydomeToggleText = document.getElementById("skydome-toggle-text") as HTMLElement;
        this.btnTogglePhysics = document.getElementById("btn-toggle-physics") as HTMLElement;
        this.physicsToggleText = document.getElementById("physics-toggle-text") as HTMLElement;
        ...
        this.btnPlay = document.getElementById("btn-play") as HTMLElement;
        this.btnPause = document.getElementById("btn-pause") as HTMLElement;
        this.btnStop = document.getElementById("btn-stop") as HTMLElement;
        this.btnSkipStart = document.getElementById("btn-skip-start") as HTMLElement;
        this.btnSkipEnd = document.getElementById("btn-skip-end") as HTMLElement;
        this.currentFrameEl = document.getElementById("current-frame") as HTMLElement;
        this.totalFramesEl = document.getElementById("total-frames") as HTMLElement;
        this.statusText = document.getElementById("status-text") as HTMLElement;
        this.statusDot = document.querySelector(".status-dot") as HTMLElement;

        // App Menu
        this.appMenu = document.getElementById("app-menu") as HTMLElement;
        this.appLogoTrigger = document.getElementById("app-logo-trigger") as HTMLElement;
        ...
        this.settingsCloseBtn = document.getElementById("settings-close") as HTMLElement;
        this.settingsBgColorInput = document.getElementById("settings-bgcolor") as HTMLInputElement;
        const initialColor = this.mmdManager.getSceneClearColor();
        this.settingsBgColorInput.value = initialColor.toHexString();

        this.viewportOverlay = document.getElementById("viewport-overlay") as HTMLElement;
        ...
        this.timelineSelectionLabel = document.getElementById("timeline-selection-label") as HTMLElement;
        ...
        this.interpolationCurveList = document.getElementById("interp-curve-list") as HTMLElement;
        ...
        this.windowTitleEl = document.getElementById("window-title");
        ...
        // Theme management
        const savedTheme = localStorage.getItem("mmd.ui.theme") || "default";
        if (this.themeSelect) {
            ...
            this.themeSelect.addEventListener("change", () => {
                ...
                localStorage.setItem("mmd.ui.theme", newTheme);
            });
        }
    ...
    private updateWindowTitle(): void {
        if (!this.windowTitleEl) return;
        const appName = "MMD modoki";
        if (this.currentProjectFilePath) {
            const fileName = this.currentProjectFilePath.replace(/^.*[\\/]/, "");
            this.windowTitleEl.textContent = `${fileName} - ${appName}`;
            document.title = `${fileName} - ${appName}`;
        } else {
            this.windowTitleEl.textContent = appName;
    ...
    private setupEventListeners(): void {
        ...
        // App Menu
        this.appLogoTrigger?.addEventListener("click", (e) => {
            e.stopPropagation();
            const currentLocale = (window as any).mmdI18n.getLocale();
            if (this.languageSelect) this.languageSelect.value = currentLocale;
            this.appMenu?.classList.toggle("hidden");
        });
        document.addEventListener("click", (e) => {
            if (this.appMenu && !this.appMenu.classList.contains("hidden") && !this.appMenu.contains(e.target as Node)) {
        ...
        menuLoadProject?.addEventListener("click", () => {
            this.appMenu?.classList.add("hidden");
            void this.loadProject();
        });
        const menuSaveProject = document.getElementById("menu-save-project");
        menuSaveProject?.addEventListener("click", () => {
            this.appMenu?.classList.add("hidden");
        ...
        const updateBgColor = () => {
            if (this.settingsBgColorInput) {
                const hex = this.settingsBgColorInput.value;
                this.mmdManager.setSceneClearColor(Color3.FromHexString(hex));
            }
        };
        this.settingsBgColorInput?.addEventListener("input", updateBgColor);
        this.settingsBgColorInput?.addEventListener("change", updateBgColor);

        this.languageSelect?.addEventListener("change", () => {
            if (this.languageSelect) {
                const newLocale = this.languageSelect.value as "ja" | "en";
                (window as any).mmdI18n.setLocale(newLocale);
            }
        });
        ...
        this.modelSelect?.addEventListener("change", () => {
            if (this.modelSelect) this.handleModelTargetSelection(this.modelSelect.value, true);
        });
        ...
        const camFov = document.getElementById("cam-fov") as HTMLInputElement | null;
        ...
        // Lighting controls
        const elAzimuth = document.getElementById("light-azimuth") as HTMLInputElement | null;
        const elElevation = document.getElementById("light-elevation") as HTMLInputElement | null;
        const elIntensity = document.getElementById("light-intensity") as HTMLInputElement | null;
        const elAmbient = document.getElementById("light-ambient") as HTMLInputElement | null;
        const elLightColorR = document.getElementById("light-color-r") as HTMLInputElement | null;
        const elLightColorG = document.getElementById("light-color-g") as HTMLInputElement | null;
        const elLightColorB = document.getElementById("light-color-b") as HTMLInputElement | null;
        const elLightFlatStrength = document.getElementById("light-flat-strength") as HTMLInputElement | null;
        const elLightFlatColorInfluence = document.getElementById("light-flat-color-influence") as HTMLInputElement | null;
        const elShadow = document.getElementById("light-shadow") as HTMLInputElement | null;
        const elShadowFrustumSize = document.getElementById("light-shadow-frustum-size") as HTMLInputElement | null;
        const elShadowColorR = document.getElementById("light-shadow-color-r") as HTMLInputElement | null;
        const elShadowColorG = document.getElementById("light-shadow-color-g") as HTMLInputElement | null;
        const elShadowColorB = document.getElementById("light-shadow-color-b") as HTMLInputElement | null;
        const elToonShadowInfluence = document.getElementById("light-toon-shadow-influence") as HTMLInputElement | null;
        const elSelfShadowSoftness = document.getElementById("light-self-shadow-softness") as HTMLInputElement | null;
        const elOcclusionShadowSoftness = document.getElementById("light-occlusion-shadow-softness") as HTMLInputElement | null;
        ...
        const updateDir = () => {
            if (elAzimuth && elElevation) {
                const az = Number(elAzimuth.value);
                const el = Number(elElevation.value);
                if (valAz) valAz.textContent = `${az} deg`;
                if (valEl) valEl.textContent = `${el} deg`;
        ...
        elIntensity?.addEventListener("input", () => {
            if (elIntensity) {
                ...
                if (valInt) valInt.textContent = v.toFixed(1);
        ...
        elAmbient?.addEventListener("input", () => {
            if (elAmbient) {
                ...
                if (valAmb) valAmb.textContent = v.toFixed(1);
                this.mmdManager.ambientIntensity = v;
            }
        });
        const applyLightColor = () => {
            if (elLightColorR && elLightColorG && elLightColorB) {
                ...
                if (valLightColorR) valLightColorR.textContent = `${Math.round(r * 100)}%`;
                if (valLightColorG) valLightColorG.textContent = `${Math.round(g * 100)}%`;
                if (valLightColorB) valLightColorB.textContent = `${Math.round(b * 100)}%`;
            }
        };
        ...
        if (elIntensity) {
            ...
            if (valInt) valInt.textContent = this.mmdManager.lightIntensity.toFixed(1);
        }
        if (elAmbient) {
            elAmbient.value = String(Math.round(this.mmdManager.ambientIntensity * 100));
            if (valAmb) valAmb.textContent = this.mmdManager.ambientIntensity.toFixed(1);
        }
        if (elLightColorR && elLightColorG && elLightColorB) {
        ...
        elShadow?.addEventListener("input", () => {
            if (elShadow) {
                ...
                if (valSh) valSh.textContent = v.toFixed(2);
                this.mmdManager.shadowDarkness = v;
            }
        });
        elShadowFrustumSize?.addEventListener("input", () => {
            if (elShadowFrustumSize) {
                const v = Number(elShadowFrustumSize.value);
                if (valShadowFrustumSize) valShadowFrustumSize.textContent = String(Math.round(v));
                this.mmdManager.shadowFrustumSize = v;
            }
        });
        const applyShadowColor = () => {
            if (elShadowColorR && elShadowColorG && elShadowColorB) {
                ...
                if (valShadowColorR) valShadowColorR.textContent = String(Math.round(r * 255));
                if (valShadowColorG) valShadowColorG.textContent = String(Math.round(g * 255));
                if (valShadowColorB) valShadowColorB.textContent = String(Math.round(b * 255));
            }
        };
        ...
        if (elShadow) {
            ...
            if (valSh) valSh.textContent = this.mmdManager.shadowDarkness.toFixed(2);
        }
        if (elShadowFrustumSize) {
            elShadowFrustumSize.value = String(Math.round(this.mmdManager.shadowFrustumSize));
            if (valShadowFrustumSize) valShadowFrustumSize.textContent = String(Math.round(this.mmdManager.shadowFrustumSize));
        }
        if (elShadowColorR && elShadowColorG && elShadowColorB) {
        ...
        if (elSelfShadowSoftness) {
            ...
            if (valSelfShSoftness) valSelfShSoftness.textContent = this.mmdManager.selfShadowEdgeSoftness.toFixed(3);
        }
        if (elOcclusionShadowSoftness) {
            elOcclusionShadowSoftness.value = String(Math.round(this.mmdManager.occlusionShadowEdgeSoftness * 1000));
            if (valOcclusionShSoftness) valOcclusionShSoftness.textContent = this.mmdManager.occlusionShadowEdgeSoftness.toFixed(3);
        }
    ...
    private setupPerfDisplay(): void {
        ...
        const updatePerfBadges = (): void => {
            ...
            if (engineEl) engineEl.textContent = engineType;
            if (shaderEl) shaderEl.textContent = shaderType;
    ...
    private setupGlobalResizeHandler(): void {
    ...
    private computeShaderMaxWidth(): number {
        ...
        return Math.max(
            UIController.MIN_SHADER_PANEL_WIDTH,
            panelWidth - shaderResizerWidth - UIController.MIN_VIEWPORT_WIDTH
        );
    }
    ...
    private updateTimelineEditState(): void {
        ...
        if (!track) {
            ...
            if (this.interpolationTrackNameLabel) this.interpolationTrackNameLabel.textContent = "-";
            if (this.interpolationFrameLabel) this.interpolationFrameLabel.textContent = "-";
            this.resetInterpolationTypeSelect();
            if (this.interpolationStatusLabel) this.interpolationStatusLabel.textContent = "No track selected";
        ...
        if (this.interpolationTrackNameLabel) this.interpolationTrackNameLabel.textContent = `${trackTypeLabel}: ${track.name}`;
        if (this.interpolationFrameLabel) this.interpolationFrameLabel.textContent = String(interpolationFrame);
    ...
    private setupPostProcessEventListeners(): void {
        const elEffectColorTemp = document.getElementById("effect-color-temp") as HTMLInputElement | null;
        const valEffectColorTemp = document.getElementById("effect-color-temp-val");
        if (elEffectColorTemp && valEffectColorTemp) {
            const applyColorTemperature = () => {
